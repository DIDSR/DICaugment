...
def test_torch_to_tensor_v2_augmentations_with_transpose_2d_mask(image, mask):
    aug = ToPytorch(transpose_mask=True)
    data = aug(image=image, mask=mask, force_apply=True)
...
def test_torch_to_tensor_v2_augmentations_with_transpose_3d_mask(image):
    aug = ToPytorch(transpose_mask=True)
    mask = np.random.randint(low=0, high=256, size=(100, 100, 50, 4), dtype=np.uint8)
    data = aug(image=image, mask=mask, force_apply=True)
    image_height, image_width, image_num_channels = image.shape
...
def test_additional_targets_for_ToPytorch():
    aug = A.Compose([ToPytorch()], additional_targets={"image2": "image", "mask2": "mask"})
    for _i in range(10):
        image1 = np.random.randint(low=0, high=256, size=(100, 100, 50), dtype=np.uint8)
        image2 = image1.copy()
        mask1 = np.random.randint(low=0, high=256, size=(100, 100, 50, 4), dtype=np.uint8)
        mask2 = mask1.copy()
...
#     message = (
#         "`ToTensor` is obsolete and it was removed from Albumentations. Please use `ToPytorch` instead - "
#         "https://albumentations.ai/docs/api_reference/pytorch/transforms/"
#         "#albumentations.pytorch.transforms.ToPytorch. "
...
#     ],
# )
# def test_brightness_contrast(brightness, contrast, saturation, hue):
#     np.random.seed(0)
#     img = np.random.randint(0, 256, [100, 100, 50], dtype=np.uint8)
...
def test_post_data_check():
    img = np.empty([100, 100, 50], dtype=np.uint8)
    bboxes = [
        [0, 0, 0, 90, 90, 90, 0],
    ]
    keypoints = [
