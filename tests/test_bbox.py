...
from albumentations import Crop, RandomCrop, RandomSizedCrop, Rotate
...
@pytest.mark.parametrize("bbox", [(15, 25, 30, 100, 200, 150), (15, 25, 30, 100, 200, 150, 99)])
...
@pytest.mark.parametrize("bbox", [(0.0375, 0.125, 0.05, 0.25, 1.0, 0.75), (0.0375, 0.125, 0.05, 0.25, 1.0, 0.75, 99)])
...
def test_denormalize_bboxes():
    bboxes = [(0.0375, 0.125, 0.05, 0.25, 1.0, 0.75), (0.0375, 0.125, 0.05, 0.25, 1.0, 0.75, 99)]
    denormalized_bboxes_1 = denormalize_bboxes(bboxes, 200, 400, 600)
    denormalized_bboxes_2 = [denormalize_bbox(bboxes[0], 200, 400, 600), denormalize_bbox(bboxes[1], 200, 400, 600)]
...
@pytest.mark.parametrize(["bboxes", "bbox_format"], [[[[20, 30, 40, 40, 50, 50]], "coco_3d"]])
