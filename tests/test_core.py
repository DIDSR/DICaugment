...
#     image_call = call(image, interpolation=1, cols=image.shape[1], rows=image.shape[0], slices=image.shape[2])
#     mask_call = call(mask, interpolation=0, cols=mask.shape[1], rows=mask.shape[0], slices=image.shape[2])
...
#     image_call = call(image, interpolation=1, cols=image.shape[1], rows=image.shape[0], slices=image.shape[2])
#     image2_call = call(mask, interpolation=0, cols=mask.shape[1], rows=mask.shape[0], slices=image.shape[2])
#     with mock.patch.object(DualTransform, "apply") as mocked_apply:
#         with mock.patch.object(DualTransform, "get_params", return_value={"interpolation": 1}):
#             aug = DualTransform(p=1)
