...
def test_rotate_interpolation(interpolation):
    ...
    expected_image = FGeometric.rotate(image, 45, interpolation=interpolation, border_mode="constant")
    expected_mask = FGeometric.rotate(mask, 45, interpolation= A.INTER_NEAREST, border_mode="constant")
...
def test_shift_scale_rotate_interpolation(interpolation):
    ...
    expected_mask = FGeometric.shift_scale_rotate(
        mask, angle=45, axes="xy", scale=2.1, dx=0.2, dy=0.2, dz=0.2, interpolation=interpolation, border_mode="constant"
    )
    assert np.array_equal(data["image"], expected_image)
    assert np.array_equal(data["mask"], expected_mask)
...
def test_force_apply():
    ...
    aug = A.Compose(
        [
            A.OneOrOther(
                A.Compose(
                    [
                        A.RandomSizedCrop(min_max_height=(64,256), height=256, width=256, depth=256, p=1),
                        A.OneOf(
                            [
                                A.RandomSizedCrop(min_max_height=(128, 256), height=64, width=64, depth=64, p=0.5),
                                A.RandomSizedCrop(min_max_height=(128, 256), height=128, width=128, depth=128, p=0.5),
                            ]
                        ),
                    ]
                ),
                A.Compose(
                    [
                        A.RandomSizedCrop(min_max_height=(128, 256), height=256, width=256, depth=256, p=1),
    ...
    res = aug(image=np.zeros((256, 256, 256), dtype=np.uint8))
    assert res["image"].shape[0] in (64, 128, 256)
    assert res["image"].shape[1] in (64, 128, 256)
    assert res["image"].shape[2] in (64, 128, 256)
...
def test_additional_targets_for_image_only(augmentation_cls, params):
    ...
    for _i in range(10):
        image1 = np.random.randint(low=0, high=256, size=(100, 100, 10), dtype=np.uint8)
        image2 = image1.copy()
...
#     img = np.random.randint(0, 256, 256 * 256 * 256, np.uint8).reshape((256, 256, 256))
...
def test_downscale(interpolation):
    img_float = np.random.rand(100, 100, 10)
...
def test_resize_keypoints():
    ...
    result = aug(image=img, keypoints=keypoints)
    assert result["keypoints"] == [(4.5, 10, 4.5, 0)]
...
#     img = np.random.randint(0, 256, [50, 10, 10], np.uint8)
#     mask = np.ones([50, 10, 10], dtype=np.int64)
...
@pytest.mark.parametrize(
    "image", [np.random.randint(0, 256, [256, 320, 100], np.uint8), np.random.random([256, 320, 100]).astype(np.float32)]
)
def test_grid_dropout_mask(image):
    mask = np.ones([256, 320, 100], dtype=np.uint8)
    ...
    mask = np.zeros([256, 320, 100], dtype=np.uint8)
    aug = A.GridDropout(p=1, mask_fill_value=0)
    result = aug(image=image, mask=mask)
    assert result["image"].sum() < image.sum()
    assert np.all(result["mask"] == 0)
    ...
    mask = np.random.randint(0, 10, [256, 320, 100], np.uint8)
...
@pytest.mark.parametrize(
    ...
    [
        (0.00001, 10, 10, 10, 100, 100, 50, 50, 50),
        (0.9, 100, None, 100, 200, None, 0, 0, 0),
        (0.4556, 10, 20, 10, None, 200, 0, 0, 0),
        (0.00004, None, None, None, 2, 100, 0, 0, 0),
    ],
)
def test_grid_dropout_params(ratio, holes_number_x, holes_number_y, holes_number_z, unit_size_min, unit_size_max, shift_x, shift_y, shift_z):
    img = np.random.randint(0, 256, [256, 320, 256], np.uint8)
    ...
    if unit_size_min and unit_size_max:
        ...
        assert (holes[0][3] - holes[0][0]) == max(1, int(ratio * 320 // holes_number_x))
        assert (holes[0][4] - holes[0][1]) == max(1, int(ratio * 256 // holes_number_y))
        assert (holes[0][5] - holes[0][2]) == max(1, int(ratio * 256 // holes_number_z))
