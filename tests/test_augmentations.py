...
@pytest.mark.parametrize(
    ...
    [
        [
            A.PadIfNeeded,
            {"min_height": 128, "min_width": 128, "min_depth": 128, "border_mode": "constant", "value": 100, "mask_value": 1},
        ],
        [A.Rotate, {"border_mode": "constant", "axes": "xy", "value": 100, "mask_value": 1}],
        [A.LongestMaxSize, {"max_size" : 50}],
        [A.SmallestMaxSize, {"max_size" : 50}],
        # [A.SafeRotate, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A.ShiftScaleRotate, {"border_mode": "constant", "value": 100, "mask_value": 1}],
...
def test_mask_fill_value(augmentation_cls, params):
    random.seed(42)
    aug = augmentation_cls(p=1, **params)
    input = {"image": np.zeros((100,100,100), dtype=np.int16) + 100, "mask": np.ones((100,100,100))}
    output = aug(**input)
...
@pytest.mark.parametrize(
    ...
    [
        [A.PadIfNeeded, {"min_height": 100, "min_width": 100, "min_depth": 100}, (50, 50, 50)],
        [A.PadIfNeeded, {"min_height": 100, "min_width": 100, "min_depth": 100}, (75, 75, 50)],
        [A.PadIfNeeded, {"min_height": 100, "min_width": 100, "min_depth": 100}, (128,128,128)],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "min_depth": None, "pad_height_divisor": 100, "pad_width_divisor": 100, "pad_depth_divisor": 100},
            (50, 50, 50),
        ],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "min_depth": None, "pad_height_divisor": 150, "pad_width_divisor": 100, "pad_depth_divisor": 75},
            (50, 100, 25),
        ],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "min_depth": None, "pad_height_divisor": 75, "pad_width_divisor": 20, "pad_depth_divisor": 30},
            (15, 15, 15),
        ],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "min_depth": None, "pad_height_divisor": 72, "pad_width_divisor": 40, "pad_depth_divisor": 36},
            (50, 60, 70),
        ],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "min_depth": None, "pad_height_divisor": 72, "pad_width_divisor": 128, "pad_depth_divisor": 64},
            (50, 50, 50),
        ],
        [A.PadIfNeeded, {"min_height": 100, "min_width": None, "min_depth": None, "pad_width_divisor": 100, "pad_depth_divisor": 100}, (50, 60, 70)],
        [A.PadIfNeeded, {"min_height": None, "min_width": 100, "min_depth": None, "pad_height_divisor": 100, "pad_depth_divisor": 100}, (50, 60, 70)],
    ],
)
...
@pytest.mark.parametrize(
    ["params", "image_shape"],
    [
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": "constant", "value": 1, "position": "center"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": "constant", "value": 1, "position": "front_top_left"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": "constant", "value": 1, "position": "front_top_right"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": "constant", "value": 1, "position": "front_bottom_left"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": "constant", "value": 1, "position": "front_bottom_right"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": "constant", "value": 1, "position": "random"}, (5, 6, 5)],
    ],
)
def test_pad_if_needed_position(params, image_shape):
    ...
    true_result = np.ones((max(image_shape[0], params["min_height"]), max(image_shape[1], params["min_width"]), max(image_shape[2], params["min_depth"])))

    if params["position"] == "center":
        x_start = image_shape[0] // 2
        y_start = image_shape[1] // 2
        z_start = image_shape[2] // 2
        true_result[x_start : x_start + image_shape[0], y_start : y_start + image_shape[1], z_start : z_start + image_shape[2]] = 0
...
def test_non_contiguous_input(augmentation_cls, params, bboxes):
    ...
    if augmentation_cls == A.RandomCropNearBBox:
        ...
        # requires "bboxes" arg
        aug = A.Compose([augmentation_cls(p=1, **params)], bbox_params=A.BboxParams(format="pascal_voc_3d"))
