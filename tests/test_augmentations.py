...
def test_augmentations_wont_change_shape_grayscale(augmentation_cls, params, image, mask):
    ...
    image = np.zeros((224, 224, 224), dtype=np.int16)
    mask = np.zeros((224, 224, 224))
    ...
    image_1ch = np.zeros((224, 224, 224, 1), dtype=np.int16)
    mask_1ch = np.zeros((224, 224, 224, 1))
...
@pytest.mark.parametrize(
    ...
    [
        [
            A.PadIfNeeded,
            {"min_height": 514, "min_width": 514, "border_mode": 0, "value": 100, "mask_value": 1},
        ],
        [A.Rotate, {"border_mode": 0, "value": 100, "mask_value": 1}],
        # [A.SafeRotate, {"border_mode": cv2.BORDER_CONSTANT, "value": 100, "mask_value": 1}],
        [A.ShiftScaleRotate, {"border_mode": 0, "value": 100, "mask_value": 1}],
...
def test_mask_fill_value(augmentation_cls, params):
    random.seed(42)
    aug = augmentation_cls(p=1, **params)
    input = {"image": np.zeros((512, 512, 512), dtype=np.int16) + 100, "mask": np.ones((512, 512, 512))}
    output = aug(**input)
...
def test_multichannel_image_augmentations(augmentation_cls, params):
    image = np.zeros((100, 100, 100, 6), dtype=np.int16)
    aug = augmentation_cls(p=1, **params)
...
def test_float_multichannel_image_augmentations(augmentation_cls, params):
    image = np.zeros((100, 100, 100, 6), dtype=np.float32)
    aug = augmentation_cls(p=1, **params)
    data = aug(image=image)
    assert data["image"].dtype == np.float32
    assert data["image"].shape[3] == 6
...
def test_multichannel_image_augmentations_diff_channels(augmentation_cls, params):
    for num_channels in range(3, 13):
        image = np.zeros((100, 100, 100, num_channels), dtype=np.int16)
        aug = augmentation_cls(p=1, **params)
        data = aug(image=image)
        assert data["image"].dtype == np.int16
...
def test_float_multichannel_image_augmentations_diff_channels(augmentation_cls, params):
    for num_channels in range(3, 13):
        image = np.zeros((100, 100, 100, num_channels), dtype=np.float32)
        aug = augmentation_cls(p=1, **params)
        data = aug(image=image)
        assert data["image"].dtype == np.float32
        assert data["image"].shape[3] == num_channels
...
@pytest.mark.parametrize(
    ...
    [
        [A.PadIfNeeded, {"min_height": 514, "min_width": 516, "min_depth": 514}, (300, 200, 100)],
        [A.PadIfNeeded, {"min_height": 514, "min_width": 516, "min_depth": 514}, (512, 516, 512)],
        [A.PadIfNeeded, {"min_height": 514, "min_width": 516, "min_depth": 514}, (600, 600, 600)],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "pad_height_divisor": 128, "pad_width_divisor": 128, "pad_depth_divisor": 128},
            (300, 200, 100),
        ],
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "pad_height_divisor": 72, "pad_width_divisor": 128, "pad_depth_divisor": 143},
            (72, 128, 143),
        ],
        [
            A.PadIfNeeded,
        ...
        [
            A.PadIfNeeded,
            {"min_height": None, "min_width": None, "pad_height_divisor": 72, "pad_width_divisor": 128, "pad_depth_divisor": 64},
            (144, 256, 32),
        ],
        [
            A.PadIfNeeded,
        ...
        [A.PadIfNeeded, {"min_height": 512, "min_width": None, "pad_width_divisor": 128, "pad_depth_divisor": 128}, (300, 200, 100)],
        [A.PadIfNeeded, {"min_height": None, "min_width": 512, "pad_height_divisor": 128, "pad_depth_divisor": 128}, (300, 200, 100)],
    ],
)
...
@pytest.mark.parametrize(
    ["params", "image_shape"],
    [
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": 0, "value": 1, "position": "center"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": 0, "value": 1, "position": "front_top_left"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": 0, "value": 1, "position": "front_top_right"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": 0, "value": 1, "position": "front_bottom_left"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": 0, "value": 1, "position": "front_bottom_right"}, (5, 6, 5)],
        [{"min_height": 10, "min_width": 12, "min_depth": 10, "border_mode": 0, "value": 1, "position": "random"}, (5, 6, 5)],
    ],
)
...
def test_non_contiguous_input(augmentation_cls, params, bboxes):
    image = np.empty([50, 100, 100], dtype=np.int16).transpose(1, 2, 0)
    mask = np.empty([50, 100, 100], dtype=np.int16).transpose(1, 2, 0)
