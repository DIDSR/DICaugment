...
class BboxParams(Params):
    ...
    Args:
        format (str): format of bounding boxes. Should be 'coco_3d', 'pascal_voc_3d', 'albumentations_3d' or 'yolo_3d'.
...
def denormalize_bbox(bbox: TBox, rows: int, cols: int, slices: int) -> TBox:
    ...
    by image height, and z-coordinates by image depth.
    This is an inverse operation for :func:`~albumentations3D.augmentations.bbox.normalize_bbox`.
...
def convert_bbox_from_albumentations(
    ...
    """Convert a bounding box from the format used by albumentations3D to a format, specified in `target_format`.

    Args:
        bbox: An albumentations3D bounding box `(x_min, y_min, z_min, x_max, y_max, z_max)`.
...
def convert_bboxes_to_albumentations(
    ...
    """Convert a list bounding boxes from a format specified in `source_format` to the format used by albumentations3d"""
    return [convert_bbox_to_albumentations(bbox, source_format, rows, cols, slices, check_validity) for bbox in bboxes]
...
def convert_bboxes_from_albumentations(
    ...
    """Convert a list of bounding boxes from the format used by albumentations3d to a format, specified
    in `target_format`.
    ...
    """
    return [convert_bbox_from_albumentations(bbox, target_format, rows, cols, slices, check_validity) for bbox in bboxes]
