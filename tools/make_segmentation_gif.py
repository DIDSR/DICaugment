...
def main():
    ...
    img = np.rot90(nib.load("./amos_data/amos_0024_image.nii.gz").get_fdata()).astype(
        np.float32
    )
    mask = np.rot90(nib.load("./amos_data/amos_0024_label.nii.gz").get_fdata()).astype(
        np.uint8
    )
    ...
    color_map = np.array(
        [(0, 0, 0, 0)] + list(map(cm, np.linspace(0, 0.9, 15)))
    ).astype("float32")[:, :3]

    tranforms = [
        A.NoOp(p=1),
        A.RandomSizedCrop(
            min_max_height=(400, 450),
            height=512,
            depth=85,
            width=512,
            d2h_ratio=0.15,
...
def gif_ify(directory, ext=".png"):
    ...
    first_frame.save(
        "segmentation_example.gif",
        format="GIF",
        append_images=frames[1:] + frames[1:-1][::-1],
        save_all=True,
        duration=100,
        loop=0,
    )
