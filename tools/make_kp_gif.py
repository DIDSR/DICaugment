...
def gif_ify(directory, ext = ".png"):
        ...
        first_frame.save("kp_example.gif", append_images = frames[1:] + frames_rev, optimize= False, save_all= True, duration = 100, loop = 0)
...
def main():
    ...
    aug = A.Compose([A.ShiftScaleRotate(
        shift_limit=0.1,
        scale_limit=(-0.1,-0.1),
        rotate_limit= (45,45),
        axes= "yz",
    ...
    for i in range(30,60):
        ...
        fig, axes = plt.subplots(1, 2, figsize= (10,10), subplot_kw=dict(projection='3d'))
        ...
        for _, (im, bb) in enumerate([(image, bboxes), (bb_im, new_bboxes)]):
            ...
            #     axes[0][_].scatter(x, y, z, c='red', s = 0.1, )
        ...
        for _, (im, kp) in enumerate([(image, keypoints), (kp_im, new_keypoints)]):
            ...
            # Plot the pixels as a scatter plot
            axes[_].scatter(x, y, z, c=colors / 255.0, s = 1.5, zorder = 2)

            kp = np.array(kp)

            x, y, z = kp[:,0], kp[:,1], kp[:,2]

            axes[_].scatter(x, y, z, c='red', s =70, alpha =1, zorder=1)

            # Set plot limits and labels
            axes[_].set_xlim(0, im.shape[0])
            axes[_].set_ylim(0, im.shape[1])
            axes[_].set_zlim(0, im.shape[2])
            axes[_].set_title(labels[_])
            axes[_].set_axis_off()
            axes[_].set_facecolor("black")
            axes[_].view_init(elev=45, azim=i, roll=0)


        fig.savefig("./out/im_{:02}.png".format(i), bbox_inches = "tight", dpi = 200)
